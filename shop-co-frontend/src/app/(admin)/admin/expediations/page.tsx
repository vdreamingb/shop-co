...
export default function ExpediationsPage():React.JSX.Element{
    ...
    const query = useQuery({queryKey: ['expediations'], queryFn: async () => await expediationsService.getAll()})

    return <>
        <h3 className="integral text-2xl">Expediations Dashboard</h3>
        <DataDisplay data={Array.isArray(query.data)? query.data: []} headers={ExpediationsTableHeaders} />
    </>
}