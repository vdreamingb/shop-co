...
export default function ReviewsPage():React.JSX.Element{
    const query = useQuery({queryKey: ['reviews'], queryFn: async () => await reviewsService.getAll()})
    return <>
        <h3 className="integral text-2xl">Reviews Dashboard</h3>
        <DataDisplay data={Array.isArray(query.data)?query.data : []} headers={reviewsTableHeadersConfig} />
    </>
}